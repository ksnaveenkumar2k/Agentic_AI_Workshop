...
# Agentic_AI_Workshop - Day 7 Module11
...
# Agentic_AI_Workshop - Day 8 Module11

# Task Name - LangGraph Assignment: Multi-Agent Research and Summarization System

**Last Updated:** 18/6/2025

# Research and Summarization Agent

This is a multi-agent research and summarization system built using LangGraph, Streamlit, and various AI tools. The application integrates a Retrieval-Augmented Generation (RAG) system with FAISS embeddings, a web research agent using the Tavily API, and a large language model (LLM) via the Google Gemini API to provide summarized responses to user queries. The system is designed to route queries to the appropriate agent (LLM, RAG, or web research) and present results in a user-friendly Streamlit interface.

## Features
- **RAG Agent**: Retrieves and summarizes information from a predefined PDF (`sample_data.pdf`) using FAISS for semantic search.
- **Web Research Agent**: Fetches real-time web data using the Tavily API for queries involving "latest" or "current" topics (e.g., news, weather, events).
- **LLM Agent**: Handles general queries with responses generated by the Gemini API.
- **Summarization**: Provides concise, neutral summaries of retrieved data using refined prompts.
- **Streamlit Interface**: Interactive frontend to input queries and view responses with the agent used.
- **Debugging Support**: Includes print statements for troubleshooting.

## Requirements
- Python 3.9 or 3.10
- Internet connection for API calls (Google Gemini and Tavily)

## Installation

### Prerequisites

1. Install Python (recommended version: 3.9 or 3.10) if not already installed.
...
create env  [ GOOGLE_API_KEY = Your Api key , TAVILY_API_KEY=your_tavily_api_key ]

Run Commands

 cd .\Agentic_AI_Workshop\Day 8\
